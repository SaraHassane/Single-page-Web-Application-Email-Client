...
function view_email(id){
  ...
  .then(email => {
      ...
      // Archive and Unarchive section
      const archive_btn = document.createElement('button');
      archive_btn.innerHTML = email.archived ? "Unarchive": "Archive";
      archive_btn.className = email.archived ? "btn btn-outline-success": "btn btn-outline-danger";
      ...
      reply_btn.className = "btn btn-outline-dark";
...
function load_mailbox(mailbox) {
  ...
  // getting the mails for the spcific mailbox and user
  fetch(`/emails/${mailbox}`)
  .then(response => response.json())
  .then(emails => {
      // creating div for mails and looping
      emails.forEach(oneMail => {
        ...
        newMail.innerHTML = `
          <h4>Sent by: ${oneMail.sender}</h4>
          <h5>Subject: ${oneMail.subject}</h5>
          <p>${oneMail.timestamp}</p>
        `;
        // Change background-color
        newMail.className = oneMail.read ? 'read': 'unread'; 
